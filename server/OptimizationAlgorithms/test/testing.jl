...
function test_algorithm(algorithm_func; func, dim=30, lower_bound=-5.0, upper_bound=5.0,
    ...
    for _ in 1:runs
        ...
        # запускаем алгоритм
        time = @elapsed begin
            best_sol, best_fit = algorithm_func(cancel_flag,
            func[1], dim, fill(lower_bound, dim), fill(upper_bound, dim), max_generations,population_size; params...)
